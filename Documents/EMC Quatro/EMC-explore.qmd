...
-   Allison advice:
    ...
    -   Pivot(?) or otherwise mutate to make a column called "collection type" with "deploy", "recovery" and in procedure, and "drone" as the sole option in drone data, so that date can be consolidated into columns called "day" "month" and "year" and be further trimmed properly
...
#### More cleaning: "collection type" and "date  

-   In this section, I'm mutating collection type columns into drone.2 and procedure.3. In order to do this properly, I need to pivot my data (I think) since right now everything is by field ID, with deploy and recover, but I want each row split in 2, and every seal will be defined across 2 rows (its recovery and its deployment)
